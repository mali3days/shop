...
export default {
  ...
  data() {
    return {
      innerFetched: false,
      ResponsiveBootstrapToolkit: window.ResponsiveBootstrapToolkit,
    };
  },
  created() {
    setTimeout(() => {
  ...
  updated() {
    ...
    const productsSliderDetail = $('.products-slider-detail');
    const itemCount = $('.products-slider-detail a').length;
    ...
    $('.products-slider-detail a').click(function anonimus() {
      ...
      const zoom = $(this).find('img').attr('data-zoom-image');
      const detail = $(this)
        .parent()
        .parent()
        .parent()
    ...
    if (this.ResponsiveBootstrapToolkit.is('>xs')) {
